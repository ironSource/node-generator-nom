...
Create an `index.js`, unless it already exists (you can optionally overwrite the existing file). To create an `index.mjs` with an ECMAScript Modules export, run either `yo nom --esm` (when running all subgenerators) or `yo nom:entrypoint --esm` (when running this subgenerator individually).
